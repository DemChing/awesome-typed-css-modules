  ...
  describe("examples", () => {
    it("should run the basic example without errors", () => {
      const result = execSync(
        `yarn typed-scss-modules 'examples/basic/**/*.scss' --includePaths examples/basic/core --aliases.~alias variables --banner '/// example banner' --no-crlf`
      ).toString();
    ...
    it("should run the default-export example without errors", () => {
      const result = execSync(
        `yarn typed-scss-modules 'examples/default-export/**/*.scss' --exportType default --nameFormat kebab --banner '/// example banner' --no-crlf`
      ).toString();
